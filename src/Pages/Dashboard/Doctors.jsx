...
const Doctors = () => {
  ...
  const registerDoctorHandler = async () => {
    ...
    const hospitalObj = await registerDoctor(doctorData, token);
    if (hospitalObj) {
  ...
  useEffect(() => {
    (async () => {
      const res = await getDoctors(hospitalData._id, token);
    ...
    <>
      <div className="w-full">
        <div className="w-full h-[15rem] py-5 justify-around sm:flex hidden">
          {cardContent.map((text, idx) => (
            <DataCard value={cardValues[idx]} text={text} key={idx} />
          ))}
        </div>
        <div className="w-full mt-24">
          <div className="w-full flex justify-between">
            <p className="font-bold text-[1.8rem] visby ml-5 sm:mb-0 mb-5 inline">
              Doctors
            </p>
            <button
          ...
          <div className="w-full flex justify-center py-10">
            <input
              className="w-10/12 h-12 rounded-3xl px-5 text-gray-500 focus:outline-none"
              placeholder="Search..."
            ></input>
          </div>
